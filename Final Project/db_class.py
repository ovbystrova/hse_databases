...
class DataBase:
    ...
    def insert_meta(self, id_video, name, date_of_recording, location, author, lang):
        self.cursor.execute(
            'INSERT INTO meta (id_video, name, date_of_recording, location, author, lang) VALUES("%d", "%s", "%s", "%s", "%s", "%s")'
            % (id_video, name, date_of_recording, location, author, lang))
        self.connection.commit()

    def insert_zhest(self, id_video, time, zhesticulation):
        self.cursor.execute('INSERT INTO zhest (id_video, time, zhesticulation) VALUES("%d", "%d", "%s")' % (
        id_video, time, zhesticulation))
        self.connection.commit()

    def insert_mimic(self, id_video, time, text):
        self.cursor.execute('INSERT INTO mimic (id_video, time, text) VALUES("%d", "%d", "%s")' % (id_video, time, text))
        self.connection.commit()

    def insert_text(self, id_video, time, text):
        self.cursor.execute('INSERT INTO text (id_video, time, text) VALUES("%d", "%d", "%s")' % (id_video, time, text))
        self.connection.commit()

    def complex_text(self, id_video, time):
        """Запрос на весь текст после такой-то секунды видео  (нужно id_video, time)."""
        try:
            self.cursor.execute('SELECT * FROM text WHERE id_video = "%d" AND time > "%d"' % (id_video, time))
...
def insert():
    if request.args:
        ...
        if function == 'insert_meta':
            ...
            author = parametrs[4]
            lang = parametrs[5]
            DB.insert_meta(id_video, name, date_of_recording, location, author, lang)
...
if __name__  == '__main__':
    parser = argparse.ArgumentParser(description='You should write USERNAME, PASSWORD, NAME')
